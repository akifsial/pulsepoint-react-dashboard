...
const SavedCommunityFeed = ({ setOpenBackFeed, setPostIdFeed, data }) => {
  const [activeTab, setActiveTab] = useState("home");
  const [activePostActions, setActivePostActions] = useState(null);
  const [isFlagModalOpen, setIsFlagModalOpen] = useState(false);
  const [showSubmitReport, setShowSubmitReport] = useState(false);
  ...
  const [shareModal, setShareModal] = useState(false);
  const [parentCommentReplyId, setParentCommentReplyId] = useState([]);
  const { data: postData, isPending: PostsPending } =
    useGetCommunityPostSaved();
  const [replyId, setReplyId] = useState();
  const myId = JSON.parse(localStorage.getItem("userInfo"));
  const [replyInput, setReplyInput] = useState("");
  ...
  return (
    ...
    //                 >
    //                   {post.userLike?.is_like === true ? (
    //                     <div className="bg-black p-1.5 cursor-pointer rounded-full">
    //                       {/* <img src={arrowUpTrans} alt="Liked" /> */}
    //                       <img
    //                         src={arrowUp}
    ...
    //                       <img
    //                         src={arrowUp}
    //                         className="rotate-180 py-[2px] px-1 cursor-pointer"
    //                         alt="Dislike"
    //                       />
    //                     </div>
    //                   ) : (
    //                     // <img src={arrowUpTrans} alt="Dislike" />
    //                     <img src={arrowDowm} className="cursor-pointer" alt="Dislike" />
    //                   )}
    //                 </button>
    //               </div>
    ...
    <div
      className={` transition-colors h-[400px] w-full duration-300 ${
        postsToRender?.records?.length > 0 ? "h-[661px] overflow-y-auto" : ""
      } `}
      // style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
    >
      {PostsPending ? (
